...
**/
if ( ! class_exists( 'spbd_Top_Bar_Walker' ) ) :
class spbd_Top_Bar_Walker extends Walker_Nav_Menu {
	private $curItem;
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$spbd_make_class = function( $classes_array, $item, $args ) {
		...
		$spbd_make_link = function( $atts, $args, $itemtitle, $itemID, $extra_data, $before_link = '' ) {
//         print_r($atts);
			$attributes = '';
...
// 			}
      if( $extra_data !== null ) $item_output .= '<small class="block subheader">' . $extra_data . '</small>';
		...
		$spbd_sort_classes = function( $all_classes ) {
		...
		switch($depth) {
			case 0:
				$all_classes = $spbd_sort_classes( $item->classes );
        if( !empty($all_classes->anc) ){
          if( empty($atts['class']) ) $atts["class"] = array();
          $atts['class'] = implode(' ', array_merge($atts['class'],$all_classes->anc));
        }
				//start with a li tag
				$output .= $indent . '<li' . $id . $spbd_make_class( $all_classes->li, $item, $args ) . ">\n";
				//add the link
				$item_output = $spbd_make_link( $atts, $args, $item->title, $item->ID, null );
				break;
			case 1:
				//is this a column
				if( !empty(array_filter($item->classes, function($check_class) {return (strpos($check_class, 'col-') !== false ? true : false);})) ) {
					...
					if( $all_classes->divider ) $output .=  "$indent\n</div>$indent\n<div" . $spbd_make_class( $all_classes->col, $item, $args ) . ">\n";
				}
        ...
        if( in_array('nat-logo-img', $item->classes) ) { $output .= '<img class="menu-column-img" src="" />'; $switch_to_img_header = true; }
				if( $args->walker->has_children ) {
					...
					if( !empty($item->title) ) $output .= $indent . '<h4' . $spbd_make_class( $item->classes, $item, $args ) . '>' . apply_filters( 'the_title', $item->title, $item->ID ) . "</h4>\n";
				} elseif( in_array('livesearch', $item->classes) ) {
					$output .= "$indent<div" . $id . $spbd_make_class( $item->classes, $item, $args ) . ">";
          ...
          if( $item->object === 'product' ) {
            ...
            if( $product_event_link !== 0 ) {
              ...
              $product_data = array();
              if( !empty($product_event->dates) ) $product_data[] = spbd_build_dates($product_event->dates, 1, true);
              ...
              if( !empty($product_data) ) $extra_data = implode(' | ', $product_data);
					...
					$output .= $spbd_make_link( $atts, $args, $item->title, $item->ID, $extra_data, $before_link );
					$output .= "\n$indent</div>\n";
				}
				$item_output = '';
				break;
        ...
        if( $item->object === 'product' ) {
          ...
          if( $product_event_link !== 0 ) {
            ...
            if( !empty($product_data) ) $extra_data = implode(' | ', $product_data);
				...
				//add the link
				$item_output = $spbd_make_link( $atts, $args, $item->title, $item->ID, $extra_data );
				break;
		}
...
if ( ! class_exists( 'spbd_Mobile_Walker' ) ) :
class spbd_Mobile_Walker extends Walker_Nav_Menu {
	function start_lvl( &$output, $depth = 0, $args = array() ) {
		$indent = str_repeat("\t", $depth);
...
if ( ! class_exists( 'spbd_Mega_Walker' ) ) :
class spbd_Mega_Walker extends Walker_Nav_Menu {
	private $curItem;
	function start_lvl( &$output, $depth = 0, $args = array() ) {
		$indent = str_repeat("\t", $depth);
		switch($depth) {
      ...
      if( $extra_data !== null ) $item_output .= '<small class="block subheader">' . $extra_data . '</small>';
        ...
        if( !empty($all_classes->anc) ){
          ...
          $atts['class'] = implode(' ', array_merge($atts['class'],$all_classes->anc));
				...
				//start with a li tag
				$output .= $indent . '<li' . $id . $spbd_make_class( $li_classes, $item, $args ) . ">\n";
				//add the link
        if( empty($atts['href']) ) {
          $item_output = $indent . '<h4' . $spbd_make_class( $atts['class'], $item, $args ) . '>' . apply_filters( 'the_title', $item->title, $item->ID ) . "</h4>\n";
        } else {
        ...
        if( in_array('nat-logo-img', $item->classes) ) { $output .= '<img class="menu-column-img" src="" />'; $switch_to_img_header = true; }
				if( $args->walker->has_children ) {
					$item->classes[] = 'nav-title' . ( ($switch_to_img_header) ? ' hide' : '');
					if( !empty($item->title) ) $output .= $indent . '<h5' . $spbd_make_class( $item->classes, $item, $args ) . '>' . apply_filters( 'the_title', $item->title, $item->ID ) . "</h5>\n";
				} elseif( in_array('livesearch', $item->classes) ) {
					$output .= "$indent<div" . $id . $spbd_make_class( $item->classes, $item, $args ) . ">";
					if( in_array('club-profiles', $item->classes) ) {
						$output .= '<h4 class="nav-title">Club Team Search</h4><span class="search-mag fi-magnifying-glass"></span><input type="text" class="search-hero g365_livesearch_input" data-g365_type="club_profiles" placeholder="Enter Team Name" autocomplete="off">';
...
if ( ! class_exists( 'spbd_Side_Slide_Walker' ) ) :
class spbd_Side_Slide_Walker extends Walker_Nav_Menu {
	private $curItem;
	function start_lvl( &$output, $depth = 0, $args = array() ) {
		$indent = str_repeat("\t", $depth);
		switch($depth) {
...
// 			}
    ...
    $before_link = '';
        ...
        if( empty($atts['href']) ) {
          $item_output = $indent . '<a' . $spbd_make_class( $atts['class'], $item, $args ) . '>' . apply_filters( 'the_title', $item_title, $item->ID ) . "</a>\n";
        } else {
          $atts['class'] = esc_attr( join( ' ', apply_filters( 'nav_menu_css_class', array_filter( $atts['class'] ), $item, $args ) ) );
          $item_output = $spbd_make_link( $atts, $args, $item_title, $item->ID, null );
        }
				break;
			case 1:
				//is this a column
        ...
        if( in_array('nat-logo-img', $item->classes) ) { $output .= '<img class="menu-column-img" src="" />'; $switch_to_img_header = true; }
				if( $args->walker->has_children ) {
					$item->classes[] = 'nav-title' . ( ($switch_to_img_header) ? ' hide' : '');
					if( !empty($item_title) ) $output .= $indent . '<h5' . $spbd_make_class( $item->classes, $item, $args ) . '>' . apply_filters( 'the_title', $item_title, $item->ID ) . "</h5>\n";
				} elseif( in_array('livesearch', $item->classes) ) {
					$output .= "$indent<div" . $id . $spbd_make_class( $item->classes, $item, $args ) . ">";
					if( in_array('club-profiles', $item->classes) ) {
						$output .= '<h4 class="nav-title">Club Team Search</h4><span class="search-mag fi-magnifying-glass"></span><input type="text" class="search-hero g365_livesearch_input" data-g365_type="club_profiles" placeholder="Enter Team Name" autocomplete="off">';
          ...
          if( in_array('event-menu-button', $item->classes) ) {
            ...
            if( count($ev_target) === 1 ) $data_tag_info = ' data-ev-target="' . substr(array_values($ev_target)[0], 10) . '"';
					...
					$output .= "$indent<div" . $id . $spbd_make_class( $item->classes, $item, $args ) . $data_tag_info . ">";
          ...
          if( in_array('event-menu-season', $item->classes) ) {
            ...
            if( $item->object === 'product' ) {
              ...
              if( !empty($product_event) ) {
                ...
                if( !empty($product_event->dates) ) $product_data['dates'][] = spbd_build_dates($product_event->dates, ( in_array('title-date', $item->classes) ) ? 2 : 1, ( in_array('title-date', $item->classes) ) ? false : true);
                $this_location = (in_array('loc-abbr', $item->classes) && !empty($product_event->short_locations)) ? $product_event->short_locations : $product_event->locations;
  ...
  //                       if( !empty($series_events[ $product_id ][ 'data_pull' ]->dates) ) $series_events[ $product_id ][ 'dates' ][] = spbd_build_dates($series_events[ $product_id ][ 'data_pull' ]->dates, 1, true);
  ...
  //               }
            ...
            $output .= $spbd_make_link( $atts, $args, $item_title, $item->ID, $extra_data, $before_link );
          }
          //close the row 
					$output .= "\n$indent</div>\n";
				}
...
//               if( !empty($product_event->logo_img) && in_array('logo-img', $item->classes) ) $before_link .= '<img class="' . ( (in_array('enhance', $item->classes)) ? 'menu-column-img' : 'menu-line-img' ) . '" src="' . $product_event->logo_img . '" />';
              $product_data = array( 'dates' => array(), 'locations' => array() );
// with date abbreviations              if( !empty($product_event->dates) ) $product_data['dates'][] = spbd_build_dates($product_event->dates, ( in_array('title-date', $item->classes) ) ? 2 : 1, ( in_array('title-date', $item->classes) ) ? false : true);
              if( !empty($product_event->dates) ) $product_data['dates'][] = spbd_build_dates($product_event->dates, 2);
              $this_location = (in_array('loc-abbr', $item->classes) && !empty($product_event->short_locations)) ? $product_event->short_locations : $product_event->locations;
              if( !empty($this_location) ) $product_data['locations'][] = implode(', ', array_map(function($val){ return explode(',', $val)[0]; }, explode('|', $this_location)));
              //see if we need to add series date info
  //             $series_ids = get_post_meta( $item->object_id, '_series_ids', true );
