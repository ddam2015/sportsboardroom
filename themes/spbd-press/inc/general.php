...
function pl_waiver_columns( $columns ) {
  ...
  $columns['spbd_user_name'] = 'User Name';
  $columns['spbd_user_email'] = 'User Email';
  return $columns;
}
add_filter( 'manage_nf_sub_posts_columns', 'pl_waiver_columns' );

function pl_waiver_column( $column, $sub_id ) {
  if( $column === 'spbd_user_name' || $column === 'spbd_user_email' ) {
    //get submission data
    $sub = Ninja_Forms()->form()->get_sub( $sub_id );
    //pull user info
    $user = $sub->get_user();
    //add it in to the column
    if( $column === 'spbd_user_name') echo $user->data->user_nicename;
    if( $column === 'spbd_user_email') echo $user->data->user_email;
  }
}
...
function add_apple_touch_icons(){
	...
	echo '<link rel="icon" type="image/png" href="' . get_template_directory_uri() . '/assets/tiny-logos/Sports-Boardroom-400x300-1.png" />' . "\n";
...
if ( ! function_exists( 'theme_setup' ) ) :
    ...
    add_theme_support( 'align-wide' );
		...
		load_theme_textdomain( 'spbd-press' );
...
if ( ! function_exists( 'spbd_main_nav' ) ) {
	function spbd_main_nav() {
    ...
    wp_nav_menu( array(
			...
			'fallback_cb'    => false,
			'walker'         => new spbd_Top_Bar_Walker()
		));
	}
}
// Main Navigation Mega
if ( ! function_exists( 'spbd_mega_nav' ) ) {
	function spbd_mega_nav() {
		wp_nav_menu( array(
			'theme_location' => 'main_nav',
			'container'      => false,
			...
			'fallback_cb'    => false,
			'walker'         => new spbd_Mega_Walker()
		));
	}
}
// Main Navigation side Slide
if ( ! function_exists( 'spbd_side_slide_nav' ) ) {
	function spbd_side_slide_nav() {
		wp_nav_menu( array(
			'theme_location' => 'main_nav',
			'container'      => false,
			...
			'fallback_cb'    => false,
			'walker'         => new spbd_Side_Slide_Walker()
		));
	}
}
// Title Nav
if ( ! function_exists( 'spbd_title_nav' ) ) {
	function spbd_title_nav() {
		wp_nav_menu( array(
			'theme_location' => 'title_nav',
			'container'      => false,
...
if ( ! function_exists( 'spbd_single_main_nav' ) ) {
  function spbd_single_main_nav() {
    wp_nav_menu( array(
      'theme_location' => 'title_nav',
      'container'      => false,
...
// Footer Nav
if ( ! function_exists( 'spbd_footer_nav' ) ) {
	function spbd_footer_nav() {
		wp_nav_menu( array(
...
if ( ! function_exists( 'spbd_excerpt' ) ) :
	/**
	 * Displays the optional excerpt.
	 * Wraps the excerpt in a div element.
	 * Create your own spbd_excerpt() function to override in a child theme.
	 * @param string $class Optional. Class string of the div element. Defaults to 'entry-summary'.
	 */
	function spbd_excerpt( $class = 'entry-summary' ) {
...
if ( ! function_exists( 'spbd_excerpt_more' ) && ! is_admin() ) :
/**
 * Replaces "[...]" (appended to automatically generated excerpts) with ... and a 'Continue reading' link.
 * Create your own spbd_excerpt_more() function to override in a child theme.
 * @return string 'Continue reading' link prepended with an ellipsis.
 */
function spbd_excerpt_more() {
	$link = sprintf( '<a href="%1$s" class="more-link">%2$s</a>',
		esc_url( get_permalink( get_the_ID() ) ),
		/* translators: %s: Name of current post */
		sprintf( __( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'spbd-press' ), get_the_title( get_the_ID() ) )
	);
	return ' &hellip; ' . $link;
}
add_filter( 'excerpt_more', 'spbd_excerpt_more' );
endif;

if ( ! function_exists( 'spbd_entry_meta' ) ) :
/**
 * Prints HTML with meta information for the categories, tags.
 * Create your own spbd_entry_meta() function to override in a child theme.
 */
function spbd_entry_meta() {
	if ( 'posts' === get_post_type() || 'careers_template' === get_post_type() ) {
		$author_avatar_size = apply_filters( 'spbd_author_avatar_size', 49 );
		printf( '<span class="byline"><span class="author vcard">%1$s <span class="screen-reader-text">%2$s </span> <a class="url fn n" href="%3$s">%4$s</a></span></span>',
			get_avatar( get_the_author_meta( 'user_email' ), $author_avatar_size ),
			_x( 'Author', 'Used before post author name.', 'spbd-press' ),
	...
	if ( current_theme_supports( 'post-formats', $format ) ) {
		printf( '<span class="entry-format">%1$s<a href="%2$s">%3$s</a></span>',
			sprintf( '<span class="screen-reader-text">%s </span>', _x( 'Format', 'Used before post format.', 'spbd-press' ) ),
...
// 		spbd_entry_taxonomies();
	}

	if ( ! is_singular() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( sprintf( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'spbd-press' ), get_the_title() ) );
		echo '</span>';
	}
}
endif;

if ( ! function_exists( 'spbd_entry_date' ) ) :
/**
 * Prints HTML with date information for current post.
 * Create your own spbd_entry_date() function to override in a child theme.
 */
function spbd_entry_date() {
...
// 		$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time> | <time class="updated" datetime="%3$s">%4$s</time>';
	...
	printf( '<span class="posted-on"><span class="screen-reader-text">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
		_x( 'Posted on', 'Used before publish date.', 'spbd-press' ),
...
function spbd_admin_menu() {
	add_menu_page( 'spbd Data Admin', 'Data', 'manage_options', 'spbd_data', 'spbd_admin', 'dashicons-chart-line', 26  );
// 	add_submenu_page( 'spbd_data', 'spbd Data Dashboard', 'Dashboard', 'manage_options', 'spbd_data', 'spbd_admin' );
}
add_action( 'admin_menu', 'spbd_admin_menu' );

//build admin page
function spbd_admin(){
	wp_enqueue_style( 'spbd_admin_styles', get_stylesheet_directory_uri() . '/css/spbd-admin-style.css' );
	...
	$spbd_sections = array(
		'settings' => array(
			...
			'tab_name'	=> 'Settings',
			'url'			=> '?page=spbd_data&g_action=settings'
		)
	);
	echo '<div class="spbd_data_manager_wrapper">';
	$spbd_admin_url = '';
	$nav = '';
	foreach( $spbd_sections as $section => $section_data ) {
		if( $section == $g_action ) {
			echo '<h1>' . $section_data['title'] . '</h1>';
			$spbd_admin_url = $section_data['url'];
	...
	echo '<nav class="nav-tab-wrapper">' . $nav . '</nav><div class="spbd_data_manager_content_wrapper">';
	switch( $g_action ) {
		case 'export_data':
			spbd_export();
			break;
		...
		case 'settings':
			...
			function spbd_update_admin_site_settings( $old, $new ) {
				foreach( $new as $item => $item_data ) {
					foreach( $item_data as $item_name => $item_value ) {
						$old[$item]['items'][$item_name]['value'] = spbd_process_data_point( $old[$item]['items'][$item_name]['type'], $item_value);
            if( $old[$item]['items'][$item_name]['value'] === null || $old[$item]['items'][$item_name]['value'] === 'null' ) $old[$item]['items'][$item_name]['value'] = '';
					}
				}
				return $old;
			}
			foreach( $spbd_admin_settings as $setting_set => &$setting_data ) {
				if( $_POST['g_process'] == 'process' ) {
					$spbd_db_option = $_POST['spbd_admin_form_data'][$setting_set];
					$update = update_option( 'spbd_' . $setting_set, $spbd_db_option );
					$setting_data['section_records'] = spbd_update_admin_site_settings( $setting_data['section_records'], $spbd_db_option );
					if( $update === true ) echo '<p class="success">Updated settings data!</p>';
				} else {
					$spbd_db_option = get_option( 'spbd_' . $setting_set );
					if( $spbd_db_option !== false && !empty($spbd_db_option) ) $setting_data['section_records'] = spbd_update_admin_site_settings( $setting_data['section_records'], $spbd_db_option );
        }
			} ?>
			<h3>General Site Settings</h3>
			<form id="spbd_form" method="post" class="spbd_form" action="<?php echo $spbd_admin_url; ?>">
				<input type="hidden" name="g_process" value="process" />
			<?php foreach( $spbd_admin_settings as $setting_set_re => $setting_data_re ) : ?>
				...
				<table class="spbd_form_section form-table">
					<tbody>
					<?php foreach( $record_data['items'] as $element => $element_data ) {
						$element_data['tag'] = 'spbd_admin_form_data[' . $setting_set_re . '][' . $record . '][' . $element . ']';
						$element_data['description'] = ( empty($element_data['description']) ) ? '' : '<small>' . $element_data['description'] . '</small>';
						echo spbd_template_construction( $element_data );
					} ?>
					</tbody>
				</table>
				<?php endforeach;
...
function g365_data_receiver(){
  $grassroots_keys = get_option( 'spbd_g365_connector' );
...
function send_html_email($email, $subject, $message) {
  $headers = ( strpos( site_url(), 'dev' ) !== false ) ? array('From: spbd Dev Customer Service <no-reply@dev.opengympremier.com>') : array('From: spbd Customer Service <no-reply@opengympremier.com>');
...
function spbd_start_ads( $pageID ){
// 	$pageID = ( $pageID === null ) ? $post->ID : $pageID;
  ...
  $spbd_page_ad = array(
	...
	$spbd_site_ads = get_option( 'spbd_display' );
	//if the page doesn't have an ad or the site global is empty, don't add to array
	$ad_info['go'] = ( empty($spbd_page_ad['link']) || empty($spbd_page_ad['img']) || empty($spbd_site_ads) ) ? false : true;
	...
	if ( $ad_info['go'] ) {
		...
		$spbd_site_ads['site_1']['element_type'] = 'rotator';
		$spbd_site_ads['site_2']['element_type'] = 'rotator';
		//build rotator element for page ad
		$ad_info['ad_content'] .= spbd_template_construction( $spbd_page_ad );
		//if we have a global ad, build and add it
		if( !empty($spbd_site_ads['site_1']['img']) ) $ad_info['ad_content'] .= spbd_template_construction( $spbd_site_ads['site_1'] );
		//if we have a secondary global ad, build if and add it along with another primary ad to help the flow
		if( !empty($spbd_site_ads['site_2']['img']) ) {
			$ad_info['ad_content'] .= spbd_template_construction( $spbd_page_ad );
			$ad_info['ad_content'] .= spbd_template_construction( $spbd_site_ads['site_2'] );
		}
	}
	//on the front page, and if we have a global splash setting
	if( is_front_page() && !empty($spbd_site_ads['site_3']) ) {
		//controls for the splash
		$spbd_splash = array(
			'title'				=> ( empty($spbd_site_ads['site_3']['title']) ) ? '' : $spbd_site_ads['site_3']['title'],
			'link'				=> ( empty($spbd_site_ads['site_3']['link']) ) ? '' : $spbd_site_ads['site_3']['link'],
			'img'					=> ( empty($spbd_site_ads['site_3']['img']) ) ? '' : $spbd_site_ads['site_3']['img'],
			'img_mobile'	=> ( empty($spbd_site_ads['site_3']['img_mobile']) ) ? '' : $spbd_site_ads['site_3']['img_mobile'],
			'description'	=> ( empty($spbd_site_ads['site_3']['description']) ) ? '' : $spbd_site_ads['site_3']['description']
		);
		//if we have minimum info process the splash
		if( !empty($spbd_splash['title']) && !empty($spbd_splash['link']) && !empty($spbd_splash['img']) ) {
			//build the splash html
			$ad_info['splash'] = '<div class="reveal text-center" id="spbd_home_reveal" aria-labelledby="' . $spbd_splash['title'] . '" data-reveal><div class="relative"><h1 id="reveal-title" class="show-for-sr">' . $spbd_splash['title'] . '</h1>';
			//if there is a derscription, add it
			if( !empty($spbd_splash['description']) ) $ad_info['splash'] .= '<p class="reveal-description">'. $spbd_splash['description'] . '</p>';
			//if there is a mobile image, add it
			if( !empty($spbd_splash['img_mobile']) ) $ad_info['splash'] .= '<p class="text-center show-for-small-only"><a href="' . $spbd_splash['link'] . '"><img src="'. $spbd_splash['img_mobile'] . '" alt="'. $spbd_splash['title'] . '" title="'. $spbd_splash['title'] . '" /></a></p>';
			//if there is a mobile image, add supporting class to the large image
			$mobile_img_class = ( empty($spbd_splash['img_mobile']) ) ? '' : ' show-for-medium';
			//add main splash image
			$ad_info['splash'] .= '<p class="text-center' . $mobile_img_class . '"><a href="' . $spbd_splash['link'] . '"><img src="'. $spbd_splash['img'] . '" alt="'. $spbd_splash['title'] . '" title="'. $spbd_splash['title'] . '" /></a></p>';
...
function spbd_template_construction( $ele_data ) {
	$spbd_admin_form_elements = array(
		'input' 	=> array(
			'template' => '<tr>
					<th>
	...
	$ele = $spbd_admin_form_elements[$ele_data['element_type']]['template'];
	foreach( $spbd_admin_form_elements[$ele_data['element_type']]['vars'] as $dex => $var ) {
    if( $var === 'options' ) {
      $all_options = '';
      $option_ele = $spbd_admin_form_elements[$ele_data['element_type']]['template_option'];
...
function spbd_process_data_point( $name, $point ) {
...
function spbd_build_dates($dates, $type = 1, $abbv = false, $add_reg = false) {
    ...
    switch( $type ){
      ...
      case 'date':
        ...
        if( empty($event_object->dates) ) {
          ...
          $date_reg = spbd_build_dates($event_object->dates, 2, false, $reg_days);
          if( $reg_days === false ) {
      ...
      case 'full_address':
        if( empty($event_object->short_locations) ) {
          ...
          $reg_days = $atts['reg_days'];
          $date_reg = spbd_build_dates($event_object->short_locations, 2, false, $reg_days);
          if( $reg_days === false ) {
            $data_compile .= $date_reg;
          } else {
            $data_compile .= $date_reg[0];
