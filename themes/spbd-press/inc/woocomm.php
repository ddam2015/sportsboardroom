...
if ( function_exists( 'is_woocommerce' ) ) {
  ...
  //woocommerce wrappers
	...
	function spbd_wrapper_start() {
		echo '<section id="content" class="grid-x site-main woocomm-wrap" role="main">';
		echo '<div class="cell small-12 large-padding">';
	}
	function spbd_wrapper_end() {
		echo '</div>';
		echo '</section>';
	}
	add_action('woocommerce_before_main_content', 'spbd_wrapper_start', 10);
	add_action('woocommerce_after_main_content', 'spbd_wrapper_end', 10);
	...
	function spbd_woocommerce_image_dimensions() {
		global $pagenow;
	...
	add_action( 'after_switch_theme', 'spbd_woocommerce_image_dimensions', 1 );

	//remove the breadcrumb (on product pages)
	function spbd_remove_wc_breadcrumbs() {
			remove_action( 'woocommerce_before_main_content', 'woocommerce_breadcrumb', 20, 0 );
	}
	add_action( 'init', 'spbd_remove_wc_breadcrumbs' );
  ...
  add_action( 'woocommerce_before_cart', 'woocommerce_checkout_login_form' );
	...
	function spbd_place_order_button_text() { return __( 'Complete Registration', 'woocommerce' ); }
	add_filter( 'woocommerce_order_button_text', 'spbd_place_order_button_text' );
  ...
  //change add to cart button text
	function spbd_woocommerce_product_add_to_cart_text(){
		return 'Register';
	}
	add_filter( 'woocommerce_product_add_to_cart_text' , 'spbd_woocommerce_product_add_to_cart_text' );
	add_filter( 'woocommerce_product_single_add_to_cart_text', 'spbd_woocommerce_product_add_to_cart_text' );
	add_filter( 'woocommerce_booking_single_add_to_cart_text', 'spbd_woocommerce_product_add_to_cart_text' );
...
function spbd_wc_scripts_styles() {
	if( is_admin() ) return;
...
add_action( 'wp_enqueue_scripts', 'spbd_wc_scripts_styles', 100);
...
function g365_add_event_id_reference() {
	wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/spbd-press/css/', 'https' ) . "style-admin.css?ver=2.4" );
...
add_filter( 'woocommerce_before_cart' , 'spbd_cart_setup');
function spbd_cart_setup() {
  //load in global proc var
  global $cart_parse;
  //process cart items
  $cart_parse = spbd_cart_process();
  //if we don't have items that need g365 data managment, and we need data collected, add it
  if( empty($cart_parse[0]) && !empty($cart_parse[2]) ){
    ...
    add_filter( 'woocommerce_after_order_notes' , function( $fields ) use ( $order_data_field ) { return spbd_player_checkout_field( $field, $order_data_field ); });
  }
}
...
function g365_order_data_form_field($no_parameter, $key, $args, $value) {
  ...
  foreach( $args['g365_cart_types'] as $dex => $type ){
    switch( $type ){
      ...
      case 'club_team':
        $current_user = wp_get_current_user();
        $presets[] = $type . '_preset,user_ac:' . ((strpos(site_url(), 'dev') === false) ? 'spbd' : 'OGD') . '-' . $current_user->ID;
        break;
    }
  }
...
function spbd_player_checkout_field( $checkout, $extra_fields = null ) {
  ...
  foreach( $extra_fields as $dex => $field_arr ) {
    ...
    $field_arr_process = array(
      ...
      'label' => $field_arr['label'] . ' for ' . $field_arr['title'],
  ...
  echo '<input type="hidden" name="spbd_extra_data" value="' . implode(',',$added_field_names) . '" />';
}

//make the extra data required based on the field names variable
add_action('woocommerce_checkout_process', 'spbd_player_checkout_field_process');
function spbd_player_checkout_field_process() {
  // Check if we have extra data
  if ( $_POST['spbd_extra_data'] ) {
    //parse and loop through the set to make sure we have all the fields filled out
    $fields = explode(',', $_POST['spbd_extra_data']);
...
add_action( 'woocommerce_checkout_update_order_meta', 'spbd_player_checkout_field_update_order_meta' );
function spbd_player_checkout_field_update_order_meta( $order_id ) {
  // Check if we have extra data
  if ( $_POST['spbd_extra_data'] ) {
    //save the field reference so we know what to pull later
    update_post_meta( $order_id, 'spbd_extra_data', sanitize_text_field( $_POST['spbd_extra_data'] ) );
...
add_action( 'woocommerce_admin_order_data_after_billing_address', 'spbd_player_checkout_field_display_admin_order_meta', 10, 1 );
function spbd_player_checkout_field_display_admin_order_meta($order){
  //parse the field names, if we have them, then echo them
  $fields = get_post_meta( $order->id, 'spbd_extra_data', true );
  if( !empty($fields) ) {
...
function spbd_edit_memberships_actions( $actions ) {
...
add_filter( 'wc_memberships_members_area_my-memberships_actions', 'spbd_edit_memberships_actions' );
add_filter( 'wc_memberships_members_area_my-membership-details_actions', 'spbd_edit_memberships_actions' );
    ...
    foreach ( $product_ids as $product_id ) {
      ...
      if( !empty($series_events[ $product_id ][ 'data_pull' ]) ) {
        ...
        if( empty($series_events[ $product_id ][ 'data_pull' ]->dates) ) {
          ...
          //add to the series order so we can be organized
          $series_order[ $product_id ] = date( 'Y-m-d', strtotime(spbd_build_dates($series_events[ $product_id ][ 'data_pull' ]->dates, 4)));
          ...
          if( $payment_options ) {
            ...
            $series_events[ $product_id ][ 'label' ][] = spbd_build_dates($series_events[ $product_id ][ 'data_pull' ]->dates, 1, true);
...
add_filter( 'woocommerce_checkout_fields' , 'spbd_modify_checkout_fields' );
// Our hooked in function - $fields is passed via the filter!
function spbd_modify_checkout_fields( $fields ) {
   unset($fields['order']['order_comments']);
   return $fields;
}

//validate the checkout form before finishing the process of writting data to spbd about the purchase
add_action('woocommerce_after_checkout_validation', 'process_spbd_data');
function process_spbd_data($form_data) {
...
add_action( 'woocommerce_checkout_update_order_meta', 'spbd_update_meta_form_fields' );
function spbd_update_meta_form_fields( $order_id ) {
...
add_action( 'show_user_profile', 'spbd_user_fields_display' );
add_action( 'edit_user_profile', 'spbd_user_fields_display' );

function spbd_user_fields_display( $user ) {
	wp_enqueue_style( 'foundation-admin', site_url( '/wp-content/themes/spbd-press/css/', 'https' ) . "style-admin.css?ver=2.345" );
	wp_enqueue_script( 'admin-jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js', array(), '2.2.4', true );
  wp_enqueue_script( 'js-g365-all', (( strpos(site_url(), 'dev.') === false ) ? 'https://grassroots365.com/data-processor.js' : 'https://dev.grassroots365.com/data-processor.js'), array('admin-jquery'), '69550', true );
  ?>
	<h3><?php esc_html_e( 'Additional User Data', 'spbd' ); ?></h3>
  <div class="options_group g365_manage_wrapper">
...
add_action( 'user_profile_update_errors', 'spbd_user_field_error', 10, 3 );
function spbd_user_field_error( $errors, $update, $user ) {
	if ( ! $update ) {
		return;
	}
}

add_action( 'personal_options_update', 'spbd_user_fields_update' );
add_action( 'edit_user_profile_update', 'spbd_user_fields_update' );

function spbd_user_fields_update( $user_id ) {
