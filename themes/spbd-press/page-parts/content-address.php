...
<article id="post-<?php the_ID(); ?>" <?php post_class('cell'); ?>>
	<header class="entry-header">
		<?php if ( is_sticky() && is_home() && !is_paged() ) : ?>
			<span class="sticky-post"><?php _e( 'Featured', 'spbd-press' ); ?></span>
		<?php endif; ?>
    ...
    remove_filter('the_content', 'foundation_content');
      /* START DEFAULT TEMPLATE */
			...
			the_content( sprintf(
				__( 'Continue reading<span class="screen-reader-text"> "%s"</span>', 'spbd-press' ),
				get_the_title()
			) );
    ...
    <?php
      ...
      wp_link_pages( array(
				'before'      => '<div class="page-links"><span class="page-links-title">' . __( 'Pages:', 'spbd-press' ) . '</span>',
				'after'       => '</div>',
				'link_before' => '<span>',
				'link_after'  => '</span>',
				'pagelink'    => '<span class="screen-reader-text">' . __( 'Page', 'spbd-press' ) . ' </span>%',
	...
	<footer class="entry-footer grid-x cell">
		...
		<?php
			edit_post_link(
				sprintf(
					/* translators: %s: Name of current post */
					__( 'Edit<span class="screen-reader-text"> "%s"</span>', 'spbd-press' ),
					get_the_title()
				),
